...
class ServiceUI extends ServiceLogic {
            ...
            <UI.TouchableOpacity onPress={() => this.navigateToReservation(item)}>
                <UI.ConnectedCbBox id="ServiceCardBox" pageId={pageId} style={styles.serviceCardBox}>
                    <UI.ConnectedCbBox id="ServiceCardImg" pageId={pageId} style={styles.serviceCardImg}>
                        <UI.ConnectedCbImage id="ServiceImg" pageId={pageId}  source={item?.ServiceClassImage == "" ? item.icon :  item.ServiceClassImage }
                        style={styles.photo}
                            imageJsx={
                                <UI.Image
                                    alt="image"
    ...
    render() {
        const { activeTab,ServiceTabHeaderConfig } = this.state;
        ...
        const activeServiceClass = this.ServiceData?.BookingTypes?.find((service) => service?.BookingTypeName === activeTab)?.ServiceClass || [];
            //console.log("$$$$ServiceMainConfig",ServiceTabHeaderConfig)
          const  ServiceTabHeaderStyles= transformStyles(ServiceTabHeaderConfig?.Styles);
                    ...
                    <UI.ScrollView horizontal>
                        {serviceCategory?.length > 0 && serviceCategory?.map((category) => {
                            return (
                                ...
                                >
                                    <UI.ConnectedCbBox id="TabHeaderBtn" pageId={pageId} 
                                    style={[ServiceTabHeaderStyles?.tabHeaderBtn || styles.tabHeaderBtn,
                                         (activeTab === category ?  ServiceTabHeaderStyles?.activeTab || styles.activeTab : {})]}>
                                        <UI.ConnectedCbText
                                           style={[ ServiceTabHeaderStyles?.tabText ||  styles.tabText,
                                                (activeTab === category ? ServiceTabHeaderStyles?.activeTabText || styles.activeTabText : {})]}
