  ...
  render() {
    ...
    if (source) {
      if (source.endsWith(".svg")) {
        ...
        return (
          <Image alt="image" source={{ uri: source }} style={this.style} />
        );
      }
    } else {
      return jsx;
          ...
          <RadioGroup>
            <View  style={{flexDirection :'row'}}>
              {selectItems.map((item, index) => (
                <Radio key={index} value={item} size="md">
                  <RadioIndicator>
                    <View  style={{width : 10, height : 10}}>
                    <RadioIcon as={CircleIcon}  color="red"/>
                    </View>
                  </RadioIndicator>
...
CbSelectDropDown.displayName = "ConnectedCbSelectDropDown";
